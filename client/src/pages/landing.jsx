...
import CustomerReviews from "../components/landing-c/testimonial";
...
function landing() {
  ...
  const { popup, handelPopcart, login, handellogin, signup, handelsignup } =
    usePopup();
  return (
    <>
      {/* Login Sign Up */}
      ...
      <img
        src={Bg}
        alt=""
        className="w-full cd:h-[100vh] ss:h-[75vh] h-[65vh] position absolute -z-10 mb-2 object-cover"
      />
      <main
        className="px-[5%] cd:h-[100vh] ss:h-[75vh] h-[65vh] bg-cover "
        id="home"
      >
        <div
          className={`flex  justify-center gap-10 max-ss:justify-between  ${
            navbarOpen ? "max-ss:items-start" : "max-ss:items-end"
          }`}
        >
          <div
            className={`text-4xl text-white mb-1 ss:hidden ${
              navbarOpen ? "mt-7" : ""
            }`}
          ...
          <div
            className={`bg-white mt-5 h-12 rounded-3xl flex items-center justify-center text-sm px-5 ${
              navbarOpen
                ? "max-ss:h-[20vh] absolute top-16 w-full text-xl left-0 flex-col"
                : "max-ss:hidden"
            }`}
          >
            <Tab btns={btns} />
      ...
      <section className="">
        <div className="h-[80vh] ss:mt-20 max-ss:mt-10 xs:px-[5%]">
          ...
          <div className="text-center m-2">
            <Link
              to="/store"
              className="py-3 px-6 rounded-3xl bg-black text-white"
            >
              {" "}
              View-More
            </Link>
